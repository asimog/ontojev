...
def test_bounded_live_run_and_separate_generation(tmp_path, monkeypatch, record_property):
    ...
    run_id = LiveOrchestrator(
        settings, repository, artifacts, jev_service=service(settings),
        deep_selection="slot:1", deep_followup_authorized=True,
    ).run()
    ...
    # Replay only from retained responses/inference. A cache miss must fail this
    ...
    replay_id = LiveOrchestrator(cached, repository, artifacts, jev_service=service(cached),
                                deep_selection="slot:1", deep_followup_authorized=True).run()
    assert repository.get_run(replay_id)["status"] == "COMPLETED"
    replay_totals = repository.gdc_run_totals(replay_id)
    ...
    original_hashes = sorted(read_state_record(repository, artifacts, row["state_id"]).scientific_hash
                             for row in repository.list_table("statistical_states", run_id))
    replay_hashes = sorted(read_state_record(repository, artifacts, row["state_id"]).scientific_hash
