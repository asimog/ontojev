...
Stage 4 systematic-discovery implementation (2026-09-25). Every claim is labeled IMPLEMENTED,
...
- IMPLEMENTED: Python domain names are unsuffixed (`StatisticalState`, `EvidenceState`,
  ...
  identity.
- IMPLEMENTED: serialized schema versions. StatisticalState 4; EvidenceState 4; ResearchSpec 4;
...
- IMPLEMENTED: one canonical `ResearchSpec`, `LUAD_RESEARCH_V1` (`domain=lung cancer`,
  ...
  `log2(UQFPKM+1)` expression summary) and the fixed `DiscoverySpec` systematic-discovery
...
- NOT IMPLEMENTED / not representable: universe enumeration beyond the fixed deterministic
  prefix (no full-genome scan, no random sample, no caller-controlled filters), an independent
  expression arm, CNV acquisition, systematic multi-lane discovery, further registered actions,
...
| Typed domain runtime (state, evidence, candidate, hypotheses, envelopes) | IMPLEMENTED; schema 4/4/4 (+ discovery result schema 1), SQLite 5, fail-closed rejection of older schemas |
...
| Systematic multi-lane discovery, independent expression arm, CNV runtime | PLANNED (Stage 5+); not current runtime |
...
Independent expression discovery over the same systematic funnel is the next task: a bounded
expression arm for the survivor set (or a declared expression universe), with its own
...
it, and do not extend the action registry or question sets, without a separately authorized task.