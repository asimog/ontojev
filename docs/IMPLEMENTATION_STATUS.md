...
- **Live orchestrator** (`cancerjev/research/live.py`): one explicit cohort (domain `lung cancer`, cohort `TCGA-LUAD`) selected by exact `project_id`; no case-count window and no multi-project pooling. A small `_acquire_project_frame()` helper owns the case frame and expression lanes, mutation discovery and count lanes, state generation, bounded budgets, and typed failure/coverage reporting.
