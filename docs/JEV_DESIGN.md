...
- **Wide evaluation:** one request per state carrying the whole `wide-v2` question set. Questions are independent; speculative answers are consumed only when applicable. The `wide-v2` set is **technically implemented but semantically stale pending redesign** for the single-cohort state (see JEV_QUESTIONS); Phase 3 is not claimed to improve a research decision.
- **Reranking:** every state in the bounded universe is evaluated; the deterministic policy orders the raw dimensions (see JEV_QUESTIONS). The deterministic baseline ranking is computed and persisted separately so the baseline vs baseline + Jev comparison can run on the same states. Raw dimensions stay visible; Jev never replaces the baseline.
