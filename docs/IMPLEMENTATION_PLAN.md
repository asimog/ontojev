...
**Implementation status (current HEAD, after the six-prompt autonomous-pipeline convergence):** the canonical systematic Campaign executor is implemented, production-wired and offline-verified: mutation discovery → expression discovery → complete CNV shard completion → terminal CNV merge → deterministic modality union → canonical `StatisticalState` persistence → `pre-wide-policy-v1` measured-evidence boundary → Wide Jev → Python admission → the autonomous candidate queue through Stage 8. The durable `program-loop-v2` worker observes the release, guards redispatch by profile/release/method identity, applies bounded deterministic retry/backoff, sleeps outside the research lock and heartbeats the canonical package version. Storage is SQLite schema 7 with sequential migrations and a read-only storage doctor; `uv.lock` is committed; strict mypy covers 85 production modules; operational logging is structured; CI runs the production-built browser path. Live-verified: the individual mutation, expression, CNV-shard, reconciliation and Jev question-set captures recorded below. Scientifically validated: **no** — `LUAD_CAMPAIGN_V1` remains `EXPERIMENTAL` and no full live Campaign with real Wide/Deep Jev has completed; readiness promotion is not implied by software tests. Deferred: Arm Jev, additional modalities, functional/targetability sources, therapeutic interpretation and any public authenticated API boundary. `GDC_FAST_SEARCH` remains only the explicitly labelled researcher/comparator path and can never produce canonical Campaign results. Post-audit hardening is also in place: `VALIDATION_RUN` activation lets an EXPERIMENTAL profile execute the identical spine without weakening the autonomous gate; one Campaign-global budget serves every lane and follow-up; expression nomination requires a real tail case; the candidate lifecycle is an enforced state machine with `CANDIDATE_COMPLETE` preserved by recovery; and `hypothesis-policy-v1` consumes recorded critique into one declared decision (currently `KEEP_HYPOTHESIS` with `NO_EVIDENCE_PRODUCING_TEST` until an evidence-producing action is registered for evidence revisions).

**Current architecture:** typed GDC requests/parsers and bounded transport; independent deterministic mutation/expression/CNV lanes; one canonical `StatisticalState` per union member with typed nominations; the persisted `pre-wide-policy-v1` boundary; Wide judgment and Python admission (`JEV_REVIEW` states are never promotable while Arm Jev is deferred); candidate-specific immutable `EvidenceState` revisions; Deep judgment and Python policies; bounded hypotheses and critique; Stage 8 final results, authoritative dossiers and deterministic no-Jev comparisons; an autonomous candidate queue that needs no operator selection; and a durable Program/Campaign worker bound to release, method and profile identity. One architecture only.

**Current scientific limitations:** the canonical Campaign spine is offline-verified only; no completed live Campaign, live Wide/Deep Jev run or Stage 8 dossier review exists, and LUAD is not validated for autonomous use. Expression tails and CNV recurrence remain descriptive (no inferential driver contract); there is no external functional/targetability axis and no therapeutic claim. Wide evaluation is bounded by the pre-Wide policy: the complete union stays persisted, any cut is recorded with explicit reasons, and ordering ties at the boundary fail closed instead of truncating.

> The P01–P19 unit sections below are the historical planning record (plan baseline `b595a3a`). Where they describe status, architecture or limitations that conflict with the current status above, the current status wins.
