...
| Control | `research/wide.py`, `deep.py`, `nextmove.py`, `investigation.py`, `program.py` | `pre-wide-policy-v1` bounds Wide by measured evidence (ties fail closed); the autonomous candidate queue needs no operator flag; durable `program-loop-v2` owns dispatch identity and bounded retry. |
