...
Campaign coordination is named, versioned and deterministic: `campaign-selection-v1` admits only profiles validated for autonomous use in declared priority-then-campaign-id order (never registry or filesystem order), `program-loop-v1` runs at most one bounded Campaign per step and records `PROGRAM_IDLE` when none is eligible, its owner-checked worker persists each step's status artifact under the autonomous run scope, `release-comparison-v1` classifies release changes from persisted snapshots with source/method changes explicitly `NOT_COMPARABLE` rather than attributed to biology, and release monitoring is a single bounded `/status` observation (`release-monitor-v1`) with typed provenance whose comparison is a separate explicit decision.
