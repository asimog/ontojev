...
Pinned references live in the untracked `.upstream/` workspace with an exact-SHA inventory (`SOURCES.lock.json`): repository URL, branch, resolved SHA, clone date, role (current reference / historical reference / executable tool / documentation only), installation status, and the reason each reference matters. Current-vs-historical workflow status follows GDC documentation and the workflow overview, not repository age, and a documented production workflow whose implementation repository is not public is recorded as such rather than substituted with a retired workflow.
...
Per current GDC workflow documentation, WXS tumor-normal calling is the four-caller MuSE / MuTect2 / VarScan2 / Pindel ensemble with an internal VCF-filtration step before VEP annotation; the SomaticSniper pipeline was deprecated at Data Release 35 (its historical files remain retrievable by UUID but are absent from current multi-caller MAFs), and no public GDC Pindel CWL repository exists. Current WGS calling is documented as VarScan2, MuTect2, Strelka2, and SvABA indels with SvABA/Manta structural variation and GATK4 CNV (replacing the Sanger CaVEMan/Pindel/BRASS/ascatNGS pipelines at Data Release 42); the production repositories for the current WGS workflow are not yet public, so its provenance must be recorded from GDC documentation and data metadata rather than from public workflow source.

The `/analysis/top_cases_counts_by_genes` gene bucket is not a distinct-case count. External reconciliation against `/ssm_occurrences` records (SCIENTIFIC_RECONCILIATION_FIXTURE `tests/reconciliation/fixtures/reconciliation_dr46`, Data Release 46.0) proved that the nested bucket value is not the number of distinct cases, not even the number of released occurrence records, and is not derivable from them: for TP53 in TCGA-LUAD the bucket reports 393 against 299 released occurrence records and 281 distinct cases, and one protein-coding gene with zero released occurrences reports a bucket of 176. Stage 4 therefore measures `MUTATION_AFFECTED_CASE_COUNT_V2`: the number of distinct cases per gene, derived locally from a complete, fail-closed `/ssm_occurrences` project scan (bounded pages of validated occurrence records). Prior Stage 4 results that ordered genes by the V1 bucket are superseded by recorded invalidation artifacts, not rewritten.
...
Current GDC expression is concurrent STAR raw counts with FPKM, FPKM-UQ, and TPM transforms under the GENCODE v36 annotation adopted at Data Release 32; reads are treated as unstranded for harmonization, and reads mapped to more than one gene are not counted. HTSeq-derived files are legacy-only (Data Release 15-31).
...
Per current GDC documentation: SNP6 CNV is produced by ASCAT2 and ASCAT3 (ASCAT3 generated by the VanLoo lab and reformatted by GDC), with gene-level copy number derived by weighted-median inheritance across overlapping segments; ABSOLUTE contributes curated gene-level copy number from the TCGA PanCancer analysis; DNAcopy/CBS segmentation derives from TCGA level-2 tangent-copy-number data; GATK4 CNV supplies current WGS segments and gene-level copy number (WGS copy-number data released as of Data Release 46.0). The ascatNGS WGS copy-number pipeline is deprecated at the GDC.
