...
|---|---|---|
| GET `/health` | none | `{status:"ok",schema_version:2}` if DB readable; 503 otherwise; does not claim worker healthy; `Cache-Control: no-store` |
...
| GET `/api/runs/{run_id}` | UUID | ResearchRun projection, last_sequence, stage occurrences, embedded candidate rows (the current candidate summary) and budget/usage snapshots |
...
| GET `/api/runs/{run_id}/evaluations` | cursor?, limit?, candidate_id?, purpose? | JevEvaluation summary/full compact vector with an explicit `input_ref_kind` (`STATISTICAL_STATE`/`EVIDENCE_STATE`/`HYPOTHESIS`) plus `input_ref_id`, applicability, mode, model/question versions |
