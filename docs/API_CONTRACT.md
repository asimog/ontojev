...
| GET `/api/states/{state_id}` | UUID | Presentation payload for one immutable StatisticalState schema-4 artifact with `state_hash`, method refs and source request refs; 404 if absent; 503 if the artifact is missing or corrupt |
