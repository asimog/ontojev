# FastAPI contract, version 3.0.0

IMPLEMENTED read API for the current typed architecture. All routes are local read APIs. JSON
...
declare `schema_version: 4` (`STATISTICAL_STATE_PRESENTATION` / `EVIDENCE_STATE_PRESENTATION`).
...
`/api/system` reports `schema_version: 5`, `mode: LIVE_AND_FIXTURE`, provider flags derived from
the environment (`TYPESAFE_API_KEY`, `OPENROUTER_API_KEY`), worker heartbeat, active run, data
sizes, configured GDC budget defaults, cache counts and
`versions: {api: "3.0.0", schema: 5, worker}`. It never reports secrets. Two labels are stale
...
|---|---|---|
| GET `/health` | none | `{status:"ok",schema_version:5}` if the DB is readable; 503 otherwise; does not claim the worker is healthy; `Cache-Control: no-store` |
...
| GET `/api/states/{state_id}` | UUID | Presentation payload for one immutable StatisticalState schema-5 artifact with `state_hash`, method refs and source request refs; 404 if absent; 503 if the artifact is missing or corrupt |
...
| GET `/api/evidence/{evidence_state_id}` | UUID | Presentation payload for one immutable EvidenceState schema-4 artifact with its source artifact identity headers; 404 if absent; 503 if the artifact is missing or corrupt |
