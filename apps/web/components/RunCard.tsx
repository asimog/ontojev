  ...
  return (
    <article className="panel run-card" data-testid={`run-${run.run_id}`}>
      <div className="row spread">
        <div><span className={`badge ${live ? "live" : "fake"}`}>{live ? "LIVE · OPEN GDC" : "FAKE · SYNTHETIC"}</span><span className={`badge status ${run.status.toLowerCase()}`}>{run.status}</span></div>
      ...
      <p className="muted">
        {live
          ? `${projectCount} project(s) · coverage ${run.coverage} · sequence ${run.last_sequence}`
          : `Fixture ${run.fixture_id} v${run.fixture_version} · ${projectCount} simulated projects · sequence ${run.last_sequence}`}
      </p>
